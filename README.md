...
* For Boundary Refinement Network (BRN), using the code in `./stage2/train/` for training. After finishing the training process of the GLN, then runt the code in `./stage1/test/test.m` to generate saliency maps of GLN. The examples image is given in `./stage2/train/data/`. Each image in the training set, including the saliency map generated by the GLN, the original RGB image, the ground truth, should be resize to 480 * 480 by the 'nearest' method. Using the code in `./stage2/train/init/generate_train.m` to generate the initialization model.
