...
* Boundary Refinement Network (BRN), using the code in `./stage2/train/` for training. The examples image is given in `./stage2/train/data/`. The training set should be resize to 480 * 480 by the 'nearest' method. Using the code in `./stage2/train/init/generate_train.m` to generate the initialization model.
